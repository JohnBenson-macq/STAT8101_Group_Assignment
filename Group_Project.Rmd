...
\begin{titlepage}
    ...
    \vspace*{1cm}
    \Huge
    \textbf{Decoding Developer Pay: An Analytical Report on the Stack Overflow 2023 Developer Survey}

    \vspace{1.5cm}
    \Large
    STAT8101 Group Project Report
    ...
    Presented for the course STAT8101
...
- Conceptualisation: Benson, John; Ilhan, Nergis; Kong, David; Koslow, Katherine; and Paul, Ayan Krishna;
- Methodology: Kong, David.;
- Software: Benson, John.;
- Validation: Benson, John; Ilhan, Nergis; Khan, Rohan Junaid; Kong, David; Koslow, Katherine; and Paul, Ayan Krishna.;
- Formal Analysis: Benson, John; Khan, Rohan Junaid; and Paul, Ayan Krishna.;
- Investigation: Benson, John; Koslow, Katherine; and Khan, Rohan Junaid.;
- Resources: Kong, David; and Koslow, Katherine.;
- Data Curation: Benson, John.;
- Writing – Original Draft Preparation: Benson, John; Ilhan, Nergis; Kong, David; and Koslow, Katherine.;
- Writing – Review & Editing: Benson, John; Ilhan, Nergis; Kong, David; Koslow, Katherine; Khan, Rohan Junaid; and Paul, Ayan Krishna.;
- Visualisation: Benson, John; Khan, Rohan Junaid; Koslow, Katherine; and Paul, Ayan Krishna.
...
5. **[Survey Methodology](#survey-methodology)**
    - [Key Principles](#key-principles)
    - [Survey Design](#survey-design)
    - [Evaluation of Methodologies](#evaluation-of-methodologies)
        - [Recruitment Methods](#recruitment-methods)
        - [Geography](#geography)
        - [Adaptive Questionnaire](#adaptive-questionnaire)
        - [Handling of Compensation Data](#handling-of-compensation-data)
        - [Technological Inclusion](#technological-inclusion)
        - [Randomisation of Questions](#randomisation-of-questions)
        - [Data Analysis Adjustments](#data-analysis-adjustments)
...
6. **[Data Analysis](#data-analysis)**
    ...
    - [Feature Selection and Data Curation](#feature-selection-and-data-curation)
...
The tech industry is characterised by rapid advancements and constant change, making it imperative to understand the determinants of developer compensation. The factors that influence compensation have far-reaching implications, not only for individual developers' career trajectories but also for the overall productivity and innovation within organisations. It is therefore vital to study the interplay between the aforementioned factors in determining compensation.
...
The data collected broadly cover four main areas of interest: respondent characteristics, technology, AI, and Stack Overflow metrics. The respondent characteristics outlined details of the respondents including age, employment status, residing country, years coding, and salary. The technology element encompassed data about the tools the respondents used as it pertains to programming including the programming languages, databases, frameworks and platforms they may use, and other tangentially related tools such as Confluence. The key topic for this year's developer survey centered around AI and, as such, several questions were asked as to the respondents' use of AI and it's effect on their programming. Finally, data was collected about the respondents' use of the Stack Overflow website including time spent on the site and what they used it for. Furthermore, several questions were asked using a Likert scale to gauge respondents' views on the design and usefulness of the site for their needs.

Given that the data set was pre-weighted along the key factor, i.e. country and that given the large number of observations in our data-set (89,000+) it was deemed unnecessary for the need to use survey packages as the design effects were likely to be minimal (Heeringa et al., 2017).

In constructing out regression model we selected predictor variables that were likely to have a strong association with the response variable, that is yearly compensation. As such it was determined that variables including the years an individual spent coding, their highest level of education attainment, their role in their organisation, the size of the organisation, the programming languages they know, the database systems they use and other tools they used were the most appropriate. All predictor variables, except for years coding, were categorical and were handled by translating each level into individual binary 1:0 variables corresponding to TRUE/FALSE respectively.
...
![Missing Data presences](Missing_data.png){width=90% height=90%}

\newpage

**Numeric Variables**

- *CompTotal*: Extremely right-skewed given outliers with extremely high compensation values.

- *ConvertedCompYearly*: Extremely right-skewed given outliers with extremely high compensation values.
...
- *convertedcomp_log*: Given the compensation outliers present within the dataset, a log transformation has been applied to the variable of interest *ConvertedCompYearly* resulting in an approximately normal distribution.

![Histogram of Numerical Variables](Numerical_variables_hist.png){width=100% height=100%}
...
- A singular *Employment* category has a large number of missing values, this is the Non-employed category as those without employment have no compensation. 
- It is evident that there is a high amount of non-response questions focused on compensation, with several *Employment* categories having only missing response variables. This is on account of the high number of unique *Employment* combinations given the multi-select string nature of the questions and the sensitivity of providing compensation information. 
...
\begin{minipage}{0.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{MV_YearsCode_vs_Pro.png}
  \caption{Yrs Coding vs Yrs Coding Professionally}
  \label{fig:figure1}
\end{minipage}\hfill
\begin{minipage}{0.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{MV_Employment_vs_converted_comp.png}
  \caption{Employment vs Compensation}
  \label{fig:figure2}
\end{minipage}
\end{figure}


UpSet plots have been used to visualise the intersections of missing data within our dataset, given several characteristics and attribute values can be encoded simultaneously (Lex et al, 2014). This plot indicates that there are higher frequencies for interceptions across a greater number of variables relating to *AI* and reduced frequencies for interceptions of various reduced combinations of *AI* attributes. Many missing values are visibly present within this dataset, therefore requiring appropriate cleaning methodologies.

![UpSet Missing values](Missing_values_upset.png){width=90% height=90%}
...
To handle missing values, we employed Multiple Imputation by Chained Equations (MICE). Five iterations were selected as it gave a balance between computational efficiency and convergence of the algorithm. For the encoded binary variables, the logistic regression method ('logreg') was utilised, for the numeric variable *YearsCode* was imputed using predictive mean matching ('pmm'), this method preserves the distributional properties of the original data (van Buuren, 2007).
...
![MICE convergence](wiggle_plots.png){width=60% height=60%}
...
In analysing compensation data, problems such as skewness and outliers was common and can have detrimental effects on the model. To address this, we performed a log-transformation on the compensation data. The transformation helped to stabilize variance and normalize the data, making it suitable for linear modeling.

The transformation's effectiveness is evident in the boxplot comparison below, which shows a marked reduction in skewness.

![Skewness of Dependent](comparison_of_skewness.png){width=50% height=50%}

To identifying the key determinants of *ConvertedCompYearly*, we utilised Lasso regression, a regularisation method designed to improve the statistical model's predictive performance and clarity. Lasso performs both variable selection and regularization, which helps in handling the multicollinearity and overfitting by shrinking less important feature's coefficients to zero. Consequently, Lasso enables us to identify the most pertinent variables while maintaining a parsimonious model.
...
![Cross Validation Results](cross_validation_results.png){width=50% height=50%}
...
Using Fisher's Exact Test with 10,000 simulated repetitions, we established a very significant connection between the two categorical variables under study (p = 9.99e-05 <0.0001). This implies that the observed link is highly unlikely to have occurred by coincidence, providing compelling evidence against the null hypothesis of no association. This conclusion might have significant consequences for our understanding of education level and organisation size. The robustness of this result is reinforced by the use of a simulation technique to account for the contingency table's particular conditions.

![Mosaic-Fisher Analyses](mosaic-fisher-plot.png){width=100% height=100%}
...
Similarily, employees with professional degrees are more common in larger organisations, although the frequency of such employees is lower in smaller organisations. The trend for developers holding associate's degrees is less obvious and medium-sized organisations exhibit a somewhat higher frequency than anticipated.
...
![Predicted vs Residuals](predicted_vs_residuals.png){width=65% height=65%}
...
Although the research is comprehensive and informative, it also comes with some limitations that should be considered when interpreting the results. Firstly, the survey is not fully randomised, as it mostly targets users who are active on Stack Overflow. This method of surveying through Stack Overflow's own channels, using onsite messaging, blog posts, and emails, can be limiting. Additionally, only two rounds of survey sending may include a limited variety of respondents.
...
Link to repository: (https://github.com/JohnBenson-macq/STAT8101_Group_Assignment)
